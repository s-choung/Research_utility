#!/usr/bin/env python3
"""
Surface Energy Analysis for Small Metal Oxide Slabs (Test Set)
==============================================================
Analyzes small slab structures from 0_slab_gen_small and calculates
surface energies to identify the most stable surfaces for each material.
...
# Calculated using FairChem UMA-S-1P1 with LBFGS relaxation (fmax=0.01 eV/Å)
BULK_ENERGIES = {
    'MgO': -11.0258,      # per MgO formula unit
    'CaO': -12.0260,      # per CaO formula unit
    'CeO2': -23.7517,     # per CeO2 formula unit
    'TiO2': -25.0039,     # per TiO2 formula unit (rutile)
    'TiO2_rutile': -25.0039,   # per TiO2 rutile formula unit
    'TiO2_anatase': -25.2051,  # per TiO2 anatase formula unit
    'SnO2': -17.2571,     # per SnO2 formula unit
    'ZnO': -8.1003,       # per ZnO formula unit
    'Al2O3': -34.8185,    # per Al2O3 formula unit
    'ZrO2': -17.3731,     # per ZrO2 formula unit
    'SiO2': -22.4021,     # per SiO2 formula unit
}


class SlabAnalyzer:
    """Analyzer for slab surface energies - adapted for small test slabs"""

    def __init__(self, slab_dirs, output_dir, log_dir):
        # Accept either a single directory or list of directories
        if isinstance(slab_dirs, (str, Path)):
            self.slab_dirs = [Path(slab_dirs)]
        else:
            self.slab_dirs = [Path(d) for d in slab_dirs]
    ...
    def parse_log_energies(self):
        """Parse energies from evaluation log file"""
        import re

        print("\n📄 Parsing energies from log file...")

        log_file = self.log_dir / "energy_evaluation.log"
    ...
    def calculate_surface_energy(self, atoms, material, miller, termination, full_material_name=None):
        """
        Calculate surface energy for a slab

        Args:
            atoms: ASE Atoms object
            material: Base material name for bulk energy lookup (e.g., 'TiO2')
            miller: Miller indices tuple
            termination: Termination index
            full_material_name: Full material name for log lookup (e.g., 'TiO2_anatase')
        ...
        try:
            ...
            # Use full material name if provided, otherwise use material
            log_material = full_material_name if full_material_name else material
    ...
    def parse_filename(self, filename):
        ...
        Examples:
            CaO_111_3x3_relaxed.traj -> material=CaO, miller=(1,1,1)
            TiO2_rutile_110_4x3_relaxed.traj -> material=TiO2_rutile, miller=(1,1,0)
            CeO2_miller100_term0_restarted.traj -> material=CeO2, miller=(1,0,0), term=0
            TiO2_rutile_miller110_term1_restarted.traj -> material=TiO2_rutile, miller=(1,1,0), term=1
        """
        stem = filename.stem  # Remove .traj extension

        # Check if this is a restarted file
        if '_restarted' in stem:
            stem = stem.replace('_restarted', '')
            parts = stem.split('_')

            # Handle materials with polymorph (e.g., TiO2_rutile, TiO2_anatase)
            ...
            # Original relaxed file format
            # Remove _relaxed suffix
            stem = stem.replace('_relaxed', '')
            parts = stem.split('_')

            # Handle materials with polymorph (e.g., TiO2_rutile, TiO2_anatase)
            if len(parts) >= 3 and parts[1] in ['rutile', 'anatase']:
                material = f"{parts[0]}_{parts[1]}"
                miller_str = parts[2]
    ...
    def analyze_slab(self, filepath):
        ...
        try:
            ...
            for frame_idx, atoms in enumerate(all_atoms):
                ...
                # Pass both base_material for bulk energy and full material name for log lookup
                result = self.calculate_surface_energy(atoms, base_material, miller, current_term,
                                                      full_material_name=material)
    ...
    def analyze_all_slabs(self):
        ...
        print("=" * 80)
        print("ANALYZING SMALL SLAB SURFACE ENERGIES")
        print("=" * 80)

        # Analyze slabs from all directories
        for slab_dir in self.slab_dirs:
            print(f"\n📁 Analyzing slabs from: {slab_dir}")

            # Process relaxed trajectory files
            relaxed_files = sorted(slab_dir.glob("*_relaxed.traj"))
            if relaxed_files:
                print(f"  Found {len(relaxed_files)} relaxed files")
                for traj_file in relaxed_files:
                    self.analyze_slab(traj_file)

            # Process restarted trajectory files
            restarted_files = sorted(slab_dir.glob("*_restarted.traj"))
            if restarted_files:
                print(f"  Found {len(restarted_files)} restarted files")
                for traj_file in restarted_files:
                    self.analyze_slab(traj_file)
    ...
    def generate_report(self):
        ...
        report_lines.append("SURFACE ENERGY ANALYSIS REPORT - SMALL SLABS")
        report_lines.append("=" * 80)
        report_lines.append("")
        ...
        # Write report
        report_file = self.output_dir / "surface_energy_report_small.txt"
    ...
    def save_json_results(self):
        ...
        json_file = self.output_dir / "surface_energies_small.json"
    ...
    def create_best_slab_paths(self):
        ...
        paths_file = self.output_dir / "best_slab_paths_small.txt"

        with open(paths_file, 'w') as f:
            f.write("# Best Surface Slab Structures - Small Slabs\n")
            f.write("# Format: Material | Miller | Surface Energy | Source | Path\n")
            f.write("#" + "=" * 78 + "\n\n")
            ...
            for material in sorted(best_surfaces.keys()):
                ...
                # Find full path by searching all slab directories
                full_path = None
                for slab_dir in self.slab_dirs:
                    candidate_path = slab_dir / best['filename']
                    if candidate_path.exists():
                        full_path = candidate_path
                        break

                if full_path is None:
                    full_path = f"NOT FOUND: {best['filename']}"

                f.write(f"{material:<15} | {miller_str:<10} | "
                       f"{best['surface_energy']:>8.4f} eV/Ų | {best['source']:<10} | {full_path}\n")

        print(f"📄 Best slab paths saved to: {paths_file}")

        return paths_file

    def create_load_best_structures(self):
        """Create a Python file with loading instructions similar to load_best_structures.py"""
        best_surfaces = self.find_best_surfaces()

        load_file = self.output_dir / "load_best_structures_small.py"

        lines = []
        lines.append('#!/usr/bin/env python3')
        lines.append('"""')
        lines.append('Load Best Surface Structures - Small Slabs (Test Set)')
        lines.append('======================================================')
        lines.append('This file defines how to load the most stable surface for each material')
        lines.append('from the small slab test set.')
        lines.append('For files with multiple terminations (frames), specifies the exact frame index.')
        lines.append('')
        lines.append('Usage:')
        lines.append('    from ase.io import read')
        lines.append('    from load_best_structures_small import BEST_STRUCTURES')
        lines.append('')
        lines.append('    # Load CeO2 best surface')
        lines.append('    atoms = read(BEST_STRUCTURES[\'CeO2\'][\'path\'],')
        lines.append('                 index=BEST_STRUCTURES[\'CeO2\'][\'frame\'])')
        lines.append('"""')
        lines.append('')
        lines.append('from pathlib import Path')
        lines.append('')
        lines.append('# Slab directories')
        for i, slab_dir in enumerate(self.slab_dirs):
            lines.append(f'SLAB_DIR_{i} = Path("{slab_dir}")')
        lines.append('')
        lines.append('# Best surface structures with frame indices')
        lines.append('BEST_STRUCTURES = {')
        ...
        for material in sorted(best_surfaces.keys()):
            ...
            lines.append(f"        'frame': {best['termination']},  # Frame {best['termination']}" +
                        (f" of {best.get('n_terminations', 1)} terminations" if best.get('n_terminations', 1) > 1 else ""))
            lines.append(f"        'miller': {best['miller_index']},")
            lines.append(f"        'termination': {best['termination']},")
            lines.append(f"        'surface_energy': {best['surface_energy']:.4f},  # eV/Å²")
            lines.append(f"        'n_terminations': {best.get('n_terminations', 1)},")
            lines.append(f"        'source': '{best['source']}',")

            # Add description
            polymorph_str = f" {best['polymorph']}" if best['polymorph'] else ""
            miller_str = ''.join(map(str, best['miller_index']))
            lines.append(f"        'description': '{material}{polymorph_str} ({miller_str}) surface'")
            lines.append('    },')
            lines.append('')
...
''')
        ...
        print(f"📄 Loading instructions saved to: {load_file}")
...
def main():
    ...
    slab_dirs = [
        base_dir / "0_slab_gen_small" / "slabs",
        base_dir / "0_slab_gen_small" / "slabs_restarted"
    ]
    log_dir = base_dir / "0_slab_gen_small"
    output_dir = base_dir / "2_slab_analysis"

    # Create analyzer
    analyzer = SlabAnalyzer(slab_dirs, output_dir, log_dir)
    ...
    load_file = analyzer.create_load_best_structures()

    print("\n" + "=" * 80)
    print("✓ ANALYSIS COMPLETE")
    print("=" * 80)
    print(f"\nResults saved in: {output_dir}")
    print(f"  - surface_energy_report_small.txt  : Detailed analysis report")
    print(f"  - surface_energies_small.json      : Machine-readable results")
    print(f"  - best_slab_paths_small.txt        : Paths to recommended structures")
    print(f"  - load_best_structures_small.py    : Python loading helper")

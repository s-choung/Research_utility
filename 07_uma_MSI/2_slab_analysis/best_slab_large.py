#!/usr/bin/env python3
"""
Surface Energy Analysis for Large Metal Oxide Slabs (Production Set)
=====================================================================
Analyzes large slab structures from 1_slab_gen and calculates
...
class SlabAnalyzer:
    """Analyzer for slab surface energies - adapted for large production slabs"""

    def __init__(self, slab_dir, output_dir, log_dir):
    ...
    def generate_report(self):
        ...
        # Write report
        report_file = self.output_dir / "surface_energy_report_large.txt"
    ...
    def save_json_results(self):
        ...
        json_file = self.output_dir / "surface_energies_large.json"
    ...
    def create_best_slab_paths(self):
        ...
        paths_file = self.output_dir / "best_slab_paths_large.txt"

        with open(paths_file, 'w') as f:
            f.write("# Best Surface Slab Structures - Large Slabs\n")
    ...
    def create_load_best_structures(self):
        ...
        load_file = self.output_dir / "load_best_structures_large.py"

        lines = []
        lines.append('#!/usr/bin/env python3')
        lines.append('"""')
        lines.append('Load Best Surface Structures - Large Slabs (Production Set)')
        lines.append('============================================================')
        lines.append('This file defines how to load the most stable surface for each material')
        lines.append('from the large slab production set.')
        ...
        lines.append('    from load_best_structures_large import BEST_STRUCTURES')
        lines.append('')
        ...
        lines.append(f'BASE_DIR = Path("{self.slab_dir}")')
        lines.append('')
        ...
        for material in sorted(best_surfaces.keys()):
            ...
            lines.append(f"        'path': str(BASE_DIR / '{best['filename']}'),")
...
def main():
    ...
    # Create analyzer
    analyzer = SlabAnalyzer(slab_dir, output_dir, log_dir)
    ...
    print(f"  - surface_energy_report_large.txt  : Detailed analysis report")
    print(f"  - surface_energies_large.json      : Machine-readable results")
    print(f"  - best_slab_paths_large.txt        : Paths to recommended structures")
    print(f"  - load_best_structures_large.py    : Python loading helper")
    print("\n✓ Using calculated bulk energies from FairChem UMA-S-1P1")

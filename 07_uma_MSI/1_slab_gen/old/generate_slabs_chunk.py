#!/usr/bin/env python3
"""
Slab Generation Script for Metal Oxide Surfaces - Chunked Version
Generates specific materials based on command line argument
"""
...
def verify_periodic_boundary(atoms):
    ...
    # Check that periodic in x,y but not z (standard for slabs)
    if not (pbc[0] and pbc[1] and not pbc[2]):
        print(f"    ⚠️  Warning: PBC should be [True, True, False], got {pbc}")
...
def generate_all_slabs_for_miller(bulk, miller_index, supercell_dimensions, thick, fixoff=False, calculator=None):
    ...
    for slab_pmg in slabs:
        ...
        # Set proper cell height: slab + vacuum (15Å sufficient for surface calculations)
        cell = slab.cell.copy()
        cell[2, 2] = slab_thickness + 17.0  # 2Å bottom + slab + 15Å vacuum
        slab.set_cell(cell, scale_atoms=False)

        # Ensure correct periodic boundary conditions (periodic in x,y only)
        ...
        # FairChem requires PBC=[True, True, True], so set it before attaching calculator
...
def main(chunk_id):
    """
    Main function to generate slabs for specific chunk of materials.

    Args:
        chunk_id: 1-5, determines which materials to process
    """

    # Define all materials
    all_materials = [
        ('MgO', mgo_bulk, (5, 5, 1), 12, [(1, 0, 0), (1, 1, 0), (1, 1, 1)]),
        ('CaO', cao_bulk, (4, 4, 1), 14, [(1, 0, 0), (1, 1, 0), (1, 1, 1)]),
        ('CeO2', ceo2_bulk, (4, 4, 1), 15, [(1, 1, 1), (1, 1, 0), (1, 0, 0)]),
        ('TiO2_rutile', tio2_rutile_bulk, (5, 4, 1), 12, [(1, 1, 0), (1, 0, 0), (1, 0, 1), (0, 0, 1)]),
        ('SnO2', sno2_bulk, (5, 4, 1), 13, [(1, 1, 0), (1, 0, 1), (1, 0, 0)]),
        ('TiO2_anatase', tio2_anatase_bulk, (6, 5, 1), 18, [(1, 0, 1), (0, 0, 1), (1, 0, 0)]),
        ('ZnO', zno_bulk, (6, 6, 1), 16, [(0, 0, 1), (1, 0, 0), (1, 1, 0)]),
        ('Al2O3', al2o3_bulk, (5, 5, 1), 20, [(0, 0, 1), (1, 0, 0), (1, 1, 0)]),
        ('ZrO2', zro2_bulk, (4, 4, 1), 18, [(1, 1, 1), (1, 0, 1), (1, 0, 0)]),
        ('SiO2', sio2_bulk, (5, 5, 1), 18, [(0, 0, 1), (1, 0, 0), (1, 0, 1)]),
    ]

    # Chunk assignment (2 materials per chunk)
    chunk_map = {
        1: [0, 1],  # MgO, CaO
        2: [2, 3],  # CeO2, TiO2_rutile
        3: [4, 5],  # SnO2, TiO2_anatase
        4: [6, 7],  # ZnO, Al2O3
        5: [8, 9],  # ZrO2, SiO2
    }

    if chunk_id not in chunk_map:
        raise ValueError(f"Invalid chunk_id: {chunk_id}. Must be 1-5.")

    material_indices = chunk_map[chunk_id]
    materials = [all_materials[i] for i in material_indices]

    print(f"\n{'='*80}")
    print(f"CHUNK {chunk_id}: Processing {len(materials)} materials")
    print(f"Materials: {', '.join([m[0] for m in materials])}")
    print(f"{'='*80}\n")
    ...
    energy_log = None
    log_filename = f'./energy_evaluation_chunk{chunk_id}.log'
    if enable_relaxation:
        energy_log = open(log_filename, 'w')
        energy_log.write("=" * 80 + "\n")
        energy_log.write(f"ENERGY EVALUATION LOG - CHUNK {chunk_id}\n")
        energy_log.write("=" * 80 + "\n\n")
        energy_log.flush()
    ...
    summary = []
    summary.append("=" * 80)
    summary.append(f"SLAB GENERATION SUMMARY - CHUNK {chunk_id}")
    summary.append("=" * 80)
    if calculator is not None:
    ...
    summary_file = f'./slab_generation_summary_chunk{chunk_id}.txt'
    ...
    if enable_relaxation:
        ...
        print(f"Energy log: {log_filename}")
    print(f"{'='*80}")


if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python generate_slabs_chunk.py <chunk_id>")
        print("chunk_id: 1 (MgO, CaO), 2 (CeO2, TiO2_rutile), 3 (SnO2, TiO2_anatase),")
        print("          4 (ZnO, Al2O3), 5 (ZrO2, SiO2)")
        sys.exit(1)
